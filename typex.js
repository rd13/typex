...
TypeX = (function() {
	...
	this.validateInput = function(pos, ori, rot, str) {
	    ...
	    if(!(/^[0-1]{5}$/).test(ori)) {
	        err.push({text:'Orientation must be 0-1 (1=reversed), and configured for 5 rotors.',
	                  val:ori});
	    }
	    //Rotors most be from 1-8, 5 in length
	    if(!(/^[0-7]{5}$/).test(pos)) {
	        err.push({text:'Selected rotors must be 0-7, and 5 rotors must be chosen.',
	                  val:pos});
	    }
	    //Rotors must be unique
	    if((/(\d)(?=.*\1)/).test(pos)) {
	        err.push({text:'Rotors must be unique.',
	                  val:pos});
	    }
	    //Rotor init positions must be alphabetical, 5 in length
	    if(!(/^[A-Z]{5}$/i).test(rot)) {
	        err.push({text:'Rotor init positions must be A-Z, and configured for 5 rotors.',
	                  val:rot});
	    }
	    //Input string must be alphabetical, spaces are OK
	    if(!(/^[A-Z ]+$/i).test(str)) {
	        err.push({text:'Input string must be A-Z, spaces are OK',
	                  val:str});
	    }
	        return err;
	}
